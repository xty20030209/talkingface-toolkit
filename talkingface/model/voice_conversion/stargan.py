...
class stargan(AbstractSpeech):
    def __init__(self, config):
        super(stargan, self).__init__()
        self.arch_type = config['arch_type']
        if self.arch_type=='conv':
            gen = Generator1(config['num_mels'], config['n_spk'], config['zdim'], config['hdim'], config['sdim'], config['normtype'], config['src_conditioning'])
        elif self.arch_type=='rnn':
            gen = Generator2(config['num_mels'], config['n_spk'], config['zdim'], config['hdim'], config['sdim'], config['src_conditioning'])
        dis = Discriminator1(config['num_mels'], config['n_spk'], config['mdim'], config['normtype'])
        self.gen = gen
        self.dis = dis
        self.n_spk = config['n_spk']
